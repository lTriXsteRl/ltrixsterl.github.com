...
function applyLocale() {
    ...
    if (locale === 'en') {
        document.title = "Service sector guide";
        $('#btnSubmit').val('Generate');
        $('#locale_flag').attr('src','/img/us.gif');
    } else {
        document.title = "Гид по сфере услуг";
...
function loadDataAfter(data) {
    ...
    const serviceSpecId = getServiceSpecID();
    if (serviceSpecId == null || serviceSpecId === '') return;
    updateUrl();
    ...
    const hintSellCnt = ((locale === 'en') ? 'Sales volume' : 'Объем продаж');
    const hintTotal = ((locale === 'en') ? 'Total' : 'Итого');

    let output_th = '';
    let output = '';
    const oavSpecProducts = oagProducts.filter((b) => oagRawMaterials[serviceSpecId].findIndex((a) => a.i === b.i) >= 0 );

    output_th += '<th id="th_city">' + ((locale === 'en') ? 'City' : 'Город') + '&nbsp;<b id="sort_by_city"></b></th>';
    output_th += '<th id="th_sell_cnt">' + hintSellCnt + '&nbsp;<b id="sort_by_sell_cnt"></b></th>';
    output_th += '<th id="th_sell_price">' + hintSellingPrice + '&nbsp;<b id="sort_by_sell_price"></b></th>';
    output_th += '<th id="th_profit">' + ((locale === 'en') ? 'Profit' : 'Прибыль') + '&nbsp;<b id="sort_by_profit"></b></th>';
    $.each(oavSpecProducts, function (cpKey, cpVal) {
    ...
    $.each(data, function (key, val) {
        let suitable = val.ssi === serviceSpecId;
        ...
        if(suitable && val.sgp != null && val.sgp.length > 0){
            ...
            output += '<a target="_blank" href="https://'+domain+'/'+realm+'/main/globalreport/marketing?geo='+val.ci+'/'+val.ri+'/'+val.ti+'&unit_type_id='+serviceID+'#by-service">'+sagTownCaption[val.ti]+'</a>';
            output += '</td>';
            let profit = 0;
            let products = '';
            $.each(oavSpecProducts, function (cpKey, cpVal) {
                const tgpVal = val.sgp.filter(function(o) { return o.pi === cpVal.i;})[0];
                if(tgpVal != null) {
            ...
            output += '<td align="right" id="td_sell_cnt" data-value="'+ parseFloat(val.sc) +'">'+ commaSeparateNumber(parseFloat(val.sc)) +'</td>';
            output += '<td align="right" id="td_sell_price" data-value="'+ parseFloat(val.sps).toFixed(2) +'">'+ commaSeparateNumber(parseFloat(val.sps).toFixed(2)) +'</td>';
            output += '<td align="right" id="td_profit" data-value="'+ parseFloat(val.iats).toFixed(2) +'">'+ commaSeparateNumber(parseFloat(val.iats).toFixed(2)) +'</td>';
            output += products;
            output += '</tr>';
        }
    });
    ...
    if(output !== '') {
        ...
        output += '<td align="right" id="td_sell_cnt_total" data-value="0">0</td>';
        output += '<td align="right" id="td_sell_price_total" data-value="0">0</td>';
    ...
    if(output !== '') {
        ...
        const nvSumSellCnt = sumArray($('td[id="td_sell_cnt"]').map((k,v) => parseFloat(v.getAttribute('data-value'))));
        $('#td_sell_cnt_total').attr('title', hintSellCnt + ': ' + commaSeparateNumber(nvSumSellCnt.toFixed(2))).text(shortenNumber(nvSumSellCnt));

        const nvSumSellPrice = sumArray($('td[id="td_sell_price"]').map((k,v) => parseFloat(v.getAttribute('data-value'))));
        $('#td_sell_price_total').attr('title', hintSumSellingPrice + ': ' + commaSeparateNumber(nvSumSellPrice.toFixed(2))).text(shortenNumber(nvSumSellPrice));
    ...
    if(output !== ''){
        ...
        setVal('sort_col_id_sg', svColId);
        setVal('sort_dir_sg', svOrder);
...
function getServiceSpecID(){
    return $('#id_service_spec').val();
}
function loadData() {
    const realm = getRealm();
    const serviceID = getServiceID();
    ...
    console.log('./'+realm+'/by_service_id/'+serviceID+'.json.zip');

    zip.workerScriptsPath = '/js/';
    zip.createReader(new zip.HttpReader('./'+realm+'/by_service_id/'+serviceID+'.json.zip'), function(reader) {
...
function loadSavedFlt(urlParams){
    ...
    let id_service = getVal('id_service');
    let id_service_spec = getVal('id_service_spec');
    ...
    const sort_col_id = urlParams['sort_col_id'] || getVal('sort_col_id_sg') || 'local_perc';
    if (sort_col_id != null || sort_col_id !== '') {
        $('#sort_col_id').val(sort_col_id);
    }
    const sort_dir = urlParams['sort_dir'] || getVal('sort_dir_sg') || 'desc';
...
// }
let oagTowns = null;
let sagTownCaption = null;
let sagCountryCaption = null;
let sagRegionCaption = null;
...
function updateUrl() {
    const serviceID = getServiceID();
    const serviceSpecID = $('#id_service_spec').val();
    const realm = getRealm();
    const svColId = $('#sort_col_id').val();
    const svOrder = $('#sort_dir').val();
    const id_country = $('#id_country').val();
...
function changeServiceSpec() {
    ...
    $.getJSON('/by_service/' + realm + '/service_unit_types' + suffix + '.json', function (data) {
    ...
    $.getJSON('/by_service/'+realm+'/service_unit_types'+suffix+'.json', function (data) {
        ...
        $.each(data, function (key, val) {
            ...
            if(selected != null && selected === val.i){
                services += ' border="1"';
                for (var k in val.s) {
                    oagRawMaterials[val.s[k].i] = val.s[k].rm;
                    serviceSpecs += '<option value="'+val.s[k].i+'">'+val.s[k].c+'</option>';
                }
            }
...
function showAllCol(){
    ...
    setVal('invisibible_columns_sg', sagInvisibibleColumns);
}
function hideAllCol(){
    $('select#show_hide_col_ru > option').each(function() {
        var value = $(this).attr('value');
...
function initShowHideColSelect() {
    ...
    show_hide_col.multiselect({
        click: function(event, ui){
            ...
            setVal('invisibible_columns_sg', sagInvisibibleColumns.filter(onlyUnique));
        },
        checkAll: function(){
            showAllCol();
        },
    ...
    sagInvisibibleColumns = getVal('invisibible_columns_sg');
...
$(document).ready(function () {
    ...
    tableHead.addEventListener('click',function(e){
        ...
        if (tableHeaderId === 'city' || tableHeaderId === 'profit' || tableHeaderId === 'sell_price' || tableHeaderId === 'sell_cnt') {
            ...
            $('#sort_dir').val(order);
            setVal('sort_col_id_sg', $('#sort_col_id').val());
            setVal('sort_dir_sg', $('#sort_dir').val());
